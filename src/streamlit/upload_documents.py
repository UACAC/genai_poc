...
def render_upload_component(
    ...
    openai_v = st.checkbox("OpenAI Vision", value=False)
    ollama_v = st.checkbox("Ollama Vision", value=False)
    hf_v = st.checkbox("HuggingFace BLIP Vision", value=False)
    enhanced_v = st.checkbox("Enhanced Vision Model", value=False)
    basic_v = st.checkbox("Basic Vision Model", value=False)
    vision_models = []
    ...
    chunk_size = st.number_input("Chunk Size", min_value=500, max_value=5000, value=1000)
    chunk_overlap = st.number_input("Chunk Overlap", min_value=0, max_value=chunk_size//2, value=200)

    if st.button("Upload and Process", key="upload_process"):
    ...
    if st.session_state.collections:
        query_collection = st.selectbox("Select Collection to Query", st.session_state.collections, key="query_collection")
        ...
        if query_text and st.button("Search Documents", key="search_documents"):
    ...
    # ---- BROWSE DOCUMENTS ----
    st.subheader("Browse Documents in Collection")
    ...
    if st.session_state.collections:
        reconstruct_collection = st.selectbox("Select Collection", st.session_state.collections, key="reconstruct_collection")
